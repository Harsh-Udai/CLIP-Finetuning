...
def evaluate_clip_on_coco(image_dir, annotation_file, device="cuda"):
    ...
    # 3. Load LoRA weights
    lora_path = os.path.join("/raid/ai23resch11003/VLM_Project/dataset_prep/test/clip_finetune_coco/saved_model/final", "lora_weights.pth")
    if os.path.exists(lora_path):
        lora_weights = torch.load(lora_path)
        model.load_state_dict(lora_weights, strict=False)
    else:
        print(f"[WARN] No lora_weights.pth found in")
